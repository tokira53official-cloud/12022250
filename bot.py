...
def get_plan_name(vps):
    """Return the premium plan name based on VPS RAM."""
    try:
        ram = int(float(str(vps.get("ram", "0GB")).upper().replace("GB", "").strip()))
    except (ValueError, TypeError):
        ram = 0
    return {4: "Dirt", 8: "Stone", 12: "Iron", 16: "Gold", 24: "Diamond", 32: "Netherite"}.get(ram, "Infinity")
        ...
        if len(vps_list) > 1:
            options = [
                discord.SelectOption(
                    label=f"VPS {i+1} ({get_plan_name(v)})",
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    creating_embed = create_embed(
        "☁️ Creating VPS",
        f"Deploying {DOCKER_IMAGE} VPS for {user.mention}",
        color=0x5865F2
    )
    creating_embed.set_footer(text=f"PrimeCloud | Op Big Node • {display_time}")
    await ctx.send(embed=creating_embed)
    ...
    try:
        ...
        # Server message: no SSH command or password.
        server_embed = create_embed("✅ VPS Created Successfully", "", color=0x00ff88)
        server_embed.add_field(name="⌯⌲ Owner", value=user.mention, inline=False)
        ...
        server_embed.add_field(name="⌯⌲ OS", value=f"`{DOCKER_IMAGE}`", inline=False)
        server_embed.set_footer(text=f"PrimeCloud | Ip Big Node • {display_time}")
        await ctx.send(embed=server_embed)

        # Initial user DM contains only VPS information and management instructions.
        # SSH/password are intentionally NOT sent here; they are generated only when
        # the user presses SSH or SSHX in 1manage.
        ...
        dm_embed.add_field(
            ...
            value=(
                ...
                f"**Configuration:** {ram} GB RAM • {cpu} CPU Cores • {disk} GB Disk\n"
